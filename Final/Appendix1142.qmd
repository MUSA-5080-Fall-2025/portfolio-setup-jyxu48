...
final_data <- bus_long %>%
  ...
  mutate(
    ...
    # 4. Fill Exposure Days
    Exposure_Days = if_else(
      is.na(Exposure_Days), 
      if_else(is_weekend == 1, 
              day_counts$n_days[day_counts$is_crime_weekend == 1], 
              day_counts$n_days[day_counts$is_crime_weekend == 0]
      ),
      Exposure_Days
    )
  ) %>%
...
```{r}
# 1. 重新定义四个模型的公式 (确保与 Phase 4 一致)
# 注意：Model 4 如果包含 factor(GEOID)，在画图时可能太慢，这里为了验证主要看前三个或简化版 Model 4
...
f3 <- Crime_Total_Count ~ Log_Ridership * is_weekend_factor + Alcohol_Count + Light_Count + Avg_Poverty + Avg_Vacancy + Avg_Income + Avg_Unemployment + offset(log(Exposure_Days))
f4 <- Crime_Total_Count ~ Log_Ridership * is_weekend_factor + Alcohol_Count + Light_Count + Avg_Poverty + Avg_Vacancy + Dist_Police + offset(log(Exposure_Days)) 
...
validation_summary <- data.frame(
  Model = c("1. Ridership Only", "2. Interaction", "3. Env & Demo", "4. Policy & Dist"),
  MAE  = c(results_m1[1], results_m2[1], results_m3[1], results_m4[1]),
  RMSE = c(results_m1[2], results_m2[2], results_m3[2], results_m4[2])
) %>%
  mutate(
